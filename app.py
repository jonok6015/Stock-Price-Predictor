...
def load_stock_data(ticker, start, end):
    try:
        # Use Ticker.history() to avoid impersonation issues
...
st.pyplot(plot_graph(
    ...
    google_data['MA_100'], 
    google_data, 
    extra_dataset=google_data['MA_250'], 
...
if len(google_data) - splitting_len <= sequence_len:
    sequence_len = max(1, len(google_data) - splitting_len - 1)
    st.warning(f"Sequence length adjusted to {sequence_len} due to limited data.")

x_test = pd.DataFrame(google_data['Close'][splitting_len:])
x_test.columns = ['Close']

if x_test.empty:
    st.error("Test dataset is empty. Cannot proceed.")
    st.stop()
...
if len(x_data) == 0:
    st.error("Not enough data to create sequences for prediction. Try a longer date range.")
    st.stop()
